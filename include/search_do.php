...
* @param array       $chunk_offset            Optional parameter used to return search results in batches. Requires two values of type int to apply this functionality.
*                                             $chunk_offset[0] is the offset of the first row to return. $chunk_offset[1] is the number of rows to return in the batch.
*                                             $chunk_offset[0] will normally be 0 in the first search, increasing by $chunk_offset[1] for each search, generated by an external looping structure.
*                                             This allows for batches of $chunk_offset[1] search results up to the total size of the search. For an example see pages/csv_export_results_metadata.php.
*                                             This approach can be used to avoid particularly large searches exceeding the PHP memory_limit when processing the data in ps_query().
*
...
function do_search(
    ...
    if (count($chunk_offset) == 2)
    {
        $results_sql->sql .= ' ' . sql_limit($chunk_offset[0], $chunk_offset[1]);
    }
