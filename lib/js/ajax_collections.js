...
function AddResourceToCollection(event,resource,size, collection_id) {
    ...
    api(
        ...
        post_data,
        function(response) {CollectionDivLoad(baseurl_short + 'pages/collections.php');}
    );
...
function RemoveSelectedFromCollection(csrf_id, csrf_token)
    ...
    CentralSpaceShowLoading();
    api("collection_remove_resources",
        post_data,
        function(response){
            if(response === true){UpdateCollectionDisplay(k);}
            else {styledalert('','Unable to remove one or more resources.');}
            CentralSpaceHideLoading();
        }
    );
