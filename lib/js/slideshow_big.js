var SlideshowTimer=0;
var SlideshowActive=false;
var SlideshowInitialised =false;
            
function RegisterSlideshowImage(image, resource, single_image_flag)
    {
    if(typeof single_image_flag === 'undefined')
        {
        single_image_flag = false;
        }

    // If we are only registering one image then remove any images registered so far
    if(single_image_flag)
        {
        SlideshowImages.length = 0;
        }

    SlideshowImages.push(image);
    }

// Used for the first cycle of the slideshow to avoid any white flashing issue in the transitions aids in cacheing
function InitialiseSlideshow()
    {
    if (SlideshowImages.length==0 || !SlideshowActive) {return false;}

    SlideshowCurrent++;  
    SlideshowNext = SlideshowCurrent+1;      

    if (SlideshowCurrent>=SlideshowImages.length)
        {
        SlideshowCurrent=0;
        SlideshowInitialised = true;
        }

    if (SlideshowNext>=SlideshowImages.length)
        {
        SlideshowNext=0;
        }

    // Using to images layered resolves flickering in transitions
    jQuery('#UICenter').css('background-image','url(' + SlideshowImages[SlideshowCurrent] + '), url(' + SlideshowImages[SlideshowNext] + ')');

    var photo_delay = 1000 * big_slideshow_timer;

    if (!SlideshowInitialised) 
        {
        SlideshowTimer=window.setTimeout(InitialiseSlideshow, photo_delay);
        }
    else 
        {
        SlideshowTimer=window.setTimeout(SlideshowChange, photo_delay);
        }
    
    return true;
    }

function SlideshowChange()
    {
    if (SlideshowImages.length==0 || !SlideshowActive) {return false;}
    
    SlideshowCurrent++;        

    if (SlideshowCurrent>=SlideshowImages.length)
        {
        SlideshowCurrent=0;
        }

    jQuery('#UICenter').css('background-image','url(' + SlideshowImages[SlideshowCurrent] + ')');

    var photo_delay = 1000 * big_slideshow_timer;
        
    SlideshowTimer=window.setTimeout(SlideshowChange, photo_delay);
    
    return true;
    }

function ActivateSlideshow()
    {
    if (!SlideshowActive)
        {
        SlideshowCurrent=-1;
        SlideshowActive=true;
        SlideshowInitialised =false;
        
        InitialiseSlideshow();
        jQuery('#Footer').hide();
        }

        jQuery( document ).ready(function() 
            {
            jQuery('#UICenter').css('transition', 'background-image 1s linear');
            });
    }
    
function DeactivateSlideshow()
    {
    jQuery('#UICenter').css('background-image','none');
    SlideshowActive=false;
    SlideshowInitialised =false;
    window.clearTimeout(SlideshowTimer);

    jQuery('#Footer').show();
    }

