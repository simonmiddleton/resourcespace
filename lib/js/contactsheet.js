...
(function($) {
	 ...
	 var methods = {
        ...
        preview : function(filename_uid)
            ...
            $.ajax({
                ...
                success: function(response)
                    {
                    $(this).rsContactSheet('refresh', response, filename_uid);
        ...
        refresh : function(pagecount, filename_uid)
            {
            document.previewimage.src = contactsheet_previewimage_prefix + '/tmp/contactsheet_' + filename_uid + '.jpg?' + Math.random();
        ...
        revert: function(filename_uid)
            {
            $('#previewpage')[0].options.length = 0;
            $('#previewpage').append(new Option(1, 1,true,true));
            $('#previewpage').value=1;
            $('#previewPageOptions').hide();
            $(this).rsContactSheet('preview', filename_uid);
            }
	};
