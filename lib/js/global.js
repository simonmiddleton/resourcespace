			...
			if (status=="error")
				...
				if (typeof chosen_config !== 'undefined' && chosen_config['#SearchBox select']!=='undefined')
					...
					jQuery('#SearchBox select').each(function()
						...
						chosen_config['#SearchBox select']['width']=css_width;
						jQuery(this).chosen(chosen_config['#SearchBox select']);
						});
					}
				}		
			});		
		...
		if (status=="error")
			...
			if (typeof chosen_config !== 'undefined' && chosen_config['#CentralSpace select']!=='undefined')
				...
				jQuery('#CentralSpace select').each(function()
					{
					var css_width = jQuery(this).css("width");
					chosen_config['#CentralSpace select']['width']=css_width;
					jQuery(this).chosen(chosen_config['#CentralSpace select']);
					});
				}
