...
*                      should at least contain load_url property
* @param {String} Optional target state toggle/open/close (default)
*
* @returns {boolean}
*/
function TogglePane(pane_id, data, target_state)
    {
    ...
    // console.log("TogglePane - target_state is " + target_state);
    ...
    jQuery("#" + pane_id).load(data.load_url, pane_data, function(response, status, xhr)
        {
        myLayout.open(active_pane);
        SetCookie('filter_state', "open");
        });
...
* @param {String} Optional target state toggle(default)|open|close 
* 
* @returns boolean
*/
function ToggleFilterBar(href, csrf_data, target_state)
    {
    if(typeof target_state === "undefined")
        {
        target_state = "toggle";
    ...
    .then(function(data)
        ...
        TogglePane('FilterBarContainer', Object.assign(csrf_data, data), target_state);
    ...
    $.fn.PutShadowOnScrollableElement = function()
        ...
        return this.each(function()
            ...
            if(element.get(0).scrollHeight <= element.get(0).clientHeight)
                {
                if(jQuery(element).hasClass("InsetBottomShadow"))
                    {
                    element.removeClass("InsetBottomShadow");
                    }
                return this;
                }
...
/**
* Helper function used to determine if filter bar is open
* 
* @return boolean
*/
