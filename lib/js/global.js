...
function ToggleBrowseBar() 
	...
	if (typeof rsbrowse === "undefined" || rsbrowse == 'hide')
		...
		//jQuery('#BrowseBarContents').show(0.2);
		rsbrowse = 'show';
		SetCookie('rsbrowse', 'show');
		}
	else
		{
		//jQuery('#BrowseBarContents').hide();
...
function renderBrowseItem(node, parent)
    ...
    brwstmplt = brwstmplt.replace("%BROWSE_LEVEL%",node.level);
    ...
    //ExpandBrowseElements('featured','featured');return false;
    //console.log(parent);
    parent.after(brwstmplt);
    //newel = jQuery(brwstmplt).insertAfter(parent)
    //console.log(brwstmplt);
    //%BROWSE_TYPE%, %BROWSE_CLASSES%, %BROWSE_LEVEL%, %BROWSE_EXPAND%, %BROWSE_NAME%, %BROWSE_LINK%, %BROWSE_LINK%, %BROWSE_ID%
    //return newel;
    }


function LoadBrowseElements(browse_id)
    {
    ...
    console.log("[data-browse-parent|='" + browse_id + "]'");
...
//TODO Reload browse_bar fucnction - call after page load if required and check cookies to reload bar and call to refresh after new restype/field/node/collection etc.