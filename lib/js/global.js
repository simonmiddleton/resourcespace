...
function ResourceSpaceAPI(name, params, callback)
    {
    postobj = {};
    for (var key in params) {
        postobj[key] = params[key];
        }

    var queryparams = jQuery.param(postobj);
    queryparams += "&function=" + name;
    queryparams += "&authmode=native";
    jQuery.ajax({
        method: 'GET',
        url: baseurl_short + 'api/?' + queryparams},
        )
        .done(function(data, textStatus, jqXHR )
            ...
            if(response=="false")
                {
                styledalert("API Error",response);
                return false;
                }
            if(isJson(response))
                {
        ...
        .fail(function(jqXHR, textStatus, errorThrown)
            {
                styledalert("API Error",textStatus);
            });
    }



function reloadcollection(response)
    {
    if(response)
        {
            CollectionDivLoad(baseurl + "/pages/collections.php",true);
        }
    }
