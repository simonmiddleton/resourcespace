...
function CentralSpaceLoad (anchor,scrolltop,modal)
	...
	CentralSpace.load(url, function (response, status, xhr)
		{
        permission_denied = 0; // If the response contains permission denied, cancel modal

        if(response.indexOf("ModalClose();") >= 0) 
            {
            status = "error";
            permission_denied = 1;
            }
        ...
        if (status=="error")
            ...
            if(permission_denied != 1) { CentralSpace.html(errorpageload  + xhr.status + " " + xhr.statusText + "<br>" + response); }
