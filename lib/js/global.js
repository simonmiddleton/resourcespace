...
function LoadActions(pagename,id,type,ref, extra_data)
    ...
    .done(function(data, textStatus, jqXHR )
        ...
        if(!response.includes('<option'))
            {
            CentralSpaceHideLoading();
            styledalert(errortext + " " + xhr.status + " " + xhr.statusText,response);
            }
        else
            {
            ...
            // The following hack is required for Chrome (tested in version 111). When rendering dropdowns in the lower part of the screen that slide upwards, the new options fail to show until the dropdown is reopened.
            setTimeout(() => {
                actionspace.css('height','auto');
                }, 100);
    ...
    .fail(function(jqXHR, textStatus, errorThrown)
        {
        CentralSpaceHideLoading();
        styledalert(errortext,errorThrown + ": " + textStatus);
        return false;
        });
    }
