...
function batch_edit_toggle_edit_multi_checkbox_question(field_ref)
    ...
    var revert = document.getElementById('revert_' + field_ref);

    // The copy_from_field is rendered with an id of the resource_type_field ref, not the field_ref (seq number) passed in here
    var this_checkbox=document.getElementById('editthis_' + field_ref);
...
/**
 * Log Javascript events relevant to ResourceSpace users' activity
 *
 * Example:
 * resourcespace_monitor_events(document.body);
 *
 * Credit: https://stackoverflow.com/a/72945018/319266 {@see https://stackoverflow.com/legal/terms-of-service/public#licensing - CC BY-SA 4.0}
 */
function resourcespace_monitor_events(element) {
    const core_events = [
        'onerror',
        'onfocus',
        'onload',
    ...
    for (const key in element) {
        if (key.startsWith('on') && core_events.indexOf(key) !== -1) {
            element.addEventListener(key.slice(2), console.debug);
        }
    }
}
