...
annotorious.plugin.RSTagging.prototype.initPlugin = function (anno)
    ...
    *****************************/
    ...
    anno.addHandler('onAnnotationCreated', function (annotation)
        ...
        // Temporary information needed on creation only:
        // Reason: annotorious does not create shapes in pixels so we need to be able to convert to pixels
        var preview_image_copy = document.getElementById('previewimagecopy');
        annotation.src_width  = preview_image_copy.width;
        annotation.src_height = preview_image_copy.height;
    ...
    anno.addHandler('onAnnotationUpdated', function (annotation)
        {
        console.warn('TODO: implement server side updating annotation!');
        });
...
annotorious.plugin.RSTagging.prototype.onInitAnnotator = function (annotator)
    ...
    // Annotorious requires an image source to function properly so we set one now
    self._IMG_SRC = jQuery('#previewimagecopy').attr('src');
        ...
        function (response)
            ...
            if(typeof response.data !== 'undefined' && response.data.length > 0)
                ...
                for(var key in response.data)
                    ...
                    response.data[key].src = self._IMG_SRC;
    ...
    jQuery(document).on('click', 'a > .RSTagging-tag', function (e)
        {
        e.preventDefault();
        ...
        */
        ...
        // Remove tag from tags if it exists
        annotation.tags = annotation.tags.filter(array_value => array_value.ref != this.dataset.ref);
        ...
        annotation.tags.push(
            ...
            resource_type_field : this.dataset.resourceTypeField,
            name                : this.dataset.name,
            parent              : this.dataset.parent,
            order_by            : this.dataset.orderBy
            });
        });
    }
