      ...
      <tbody>
         ...
         foreach ($inst_plugins as $p)
            ...
            if (isset($p['legacy_inst']))
               {
               echo '<a class="nowrap" href="#"><?php echo LINK_CARET ?>'.$lang['plugins-legacyinst'].'</a> '; # TODO: Update this link to point to a help page on the wiki
               }
            else
               {
               echo '<a href="#'.$p['name'].'" class="p-deactivate"><?php echo LINK_CARET ?>'.$lang['plugins-deactivate'].'</a> ';
               }
            if ($p['info_url']!='')
               {
               echo '<a class="nowrap" href="'.$p['info_url'].'" target="_blank"><?php echo LINK_CARET ?>'.$lang['plugins-moreinfo'].'</a> ';
               }
            echo '<a onClick="return CentralSpaceLoad(this,true);" class="nowrap" href="'.$baseurl_short.'pages/team/team_plugins_groups.php?plugin=' . urlencode($p['name']) . '"><?php echo LINK_CARET ?>'.$lang['groupaccess'].'</a> ';
            $p['enabled_groups'] = array($p['enabled_groups']);
            if ($p['config_url']!='')        
               {
               if(($p['enabled_groups'][0]=='' ||  in_array($userdata[0]['usergroup'],explode(",",$p['enabled_groups'][0]))))
                  {
                  echo '<a onClick="return CentralSpaceLoad(this,true);" class="nowrap" href="'.$baseurl.$p['config_url'].'"><?php echo LINK_CARET ?>'.$lang['options'].'</a> ';        
                  if (sql_value("SELECT config_json as value from plugins where name='".$p['name']."'",'')!='' && function_exists('json_decode'))
                     {
                     echo '<a class="nowrap" href="'.$baseurl_short.'pages/team/team_download_plugin_config.php?pin='.$p['name'].'"><?php echo LINK_CARET ?>'.$lang['plugins-download'].'</a> ';
                     }
                  }
               else
                  {
                  echo '<?php echo LINK_CARET ?><span class="nowrap" style="text-decoration: line-through;cursor:not-allowed;">'.$lang['options'].'</span> '; 
                  }
               }
            echo '</div></td></tr>';
            } 
...
if (count($plugins_avail)>0) 
   ...
   foreach($plugins_avail as $p)
      ...
      $plugin_row .= '<a href="#'.$p['name'].'" class="p-activate"><?php echo LINK_CARET ?>'.$lang['plugins-activate'].'</a> ';
      if ($p['info_url']!='')
         {
         $plugin_row .= '<a class="nowrap" href="'.$p['info_url'].'" target="_blank"><?php echo LINK_CARET ?>'.$lang['plugins-moreinfo'].'</a> ';
         }
      if ($p['config'])
         {
         $plugin_row .= '<a href="#'.$p['name'].'" class="p-purge"><?php echo LINK_CARET ?>'.$lang['plugins-purge'].'</a> ';
         }
