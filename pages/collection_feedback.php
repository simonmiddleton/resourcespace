...
<?php if ($feedback_resource_select)
	...
	for ($n=0;$n<count($result);$n++)
		...
		?><a class="lightbox-feedback" href="<?php echo escape_quoted_data($path)?>" title="<?php echo escape_quoted_data($displaytitle) ?>"><img width="<?php echo (int) $result[$n]["thumb_width"]?>" height="<?php echo (int) $result[$n]["thumb_height"]?>" src="<?php echo escape_quoted_data(get_resource_path($ref,false,"thm",false,$result[$n]["preview_extension"],-1,1,(checkperm("w") || ($k!="" && $watermark !== "")) && $access==1,$result[$n]["file_modified"]))?>" class="ImageBorder"></a>
