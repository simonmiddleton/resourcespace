...
<?php if ($sent) { ?><p><?php echo escape($lang["feedback_thank_you"]); ?></p><?php 
} else { ?>
...
<?php } ?>
        ...
        <?php foreach (explode(",",$feedback_questions[$n]["options"]) as $option)
            {
            ?>
            <tr><td width="1"><input type="radio" name="question_<?php echo $n?>" value="<?php echo escape($option);?>" <?php if ($option==getval("question_" . $n,"")) { ?>checked<?php } ?>></td><td><?php echo escape($option);?></td></tr>
            <?php
            }
        ?>
        </table>
<?php } ?>
        ...
        <?php $opt=0;foreach (explode(",",$feedback_questions[$n]["options"]) as $option)
            {
            ?>
            <tr><td width="1"><input type="checkbox" name="question_<?php echo $n?>_<?php echo $opt?>" value="yes" <?php if (getval("question_" . $n . "_" . $opt,"")!="") { ?>checked<?php } ?>></td><td><?php echo escape($option);?></td></tr>
            <?php
            $opt++;
            }
        ?>
