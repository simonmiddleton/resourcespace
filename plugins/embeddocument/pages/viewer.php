...
<li class="embeddocument_auto" Style="cursor: pointer;" onClick="embeddocument_auto=!embeddocument_auto;if (embeddocument_auto) {embeddocument_ShowPage(embeddocument_page,false,false_<?php echo escape($ref); ?>);} else {clearTimeout(timer);}"<span>||</span></li>
...
<?php if ($downloadkey != "")
    ...
    if($arrkeydata[0] == $ak && $arrkeydata[1] == $k &&  $arrkeydata[2] == $ref)
        ...
        if (file_exists($pdf_file_path))
            ...
            ?>
            <li class="embeddocument_download" Style="cursor: pointer;" onClick="top.location.href='<?php echo $pdf_url_path ?>';"><?php echo escape($lang["embeddocument_download_pdf"]); ?></li>
            <?php
            }
        }
    }
...
function embeddocument_ShowPage(page_set,from_auto,jump)
    ...
    if (embeddocument_page==page_set && jump) {alert("<?php echo escape($lang["embeddocument_alreadyonpage"]); ?>");return false;}
