	...
	if($errortext=="")
		...
		if($code=="new")
			...
			$max=sql_value("select max(code) as value from archive_states","");
                        $code=$max+1;
			sql_query("insert into archive_states (code,name,notify_group,more_notes_flag,notify_user_flag,email_from,bcc_admin) values('$code','$name','$notify_group','$more_notes','$notify_user','$rse_workflow_email_from','$rse_workflow_bcc_admin')");
			}
		else
			{
			sql_query("update archive_states set name='$name',notify_group='$notify_group',more_notes_flag='$more_notes',notify_user_flag='$notify_user', email_from='$rse_workflow_email_from',bcc_admin='$rse_workflow_bcc_admin' where code='$code'");
			}
		$saved=true;
		}
