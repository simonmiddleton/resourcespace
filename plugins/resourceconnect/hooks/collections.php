...
function HookResourceconnectCollectionsThumblistextra()
	...
	foreach ($thumbs as $thumb)
		...
		<a href="<?php echo $baseurl ?>/plugins/resourceconnect/pages/view.php?url=<?php echo urlencode($thumb["url"]) ?>&k=<?php echo htmlspecialchars(getval("k","")) ?>&col=<?php echo $usercollection ?>" onClick="return ModalLoad(this,true);"><div class="overlay-link-container"><i class="overlay-link fas fa-link"></i></div><img border=0 src="<?php echo $thumb["thumb"] ?>" class="CollectImageBorder" /></a></td> 
		</tr></table>

		<div class="CollectionPanelInfo"><a href="<?php echo $baseurl ?>/plugins/resourceconnect/pages/view.php?url=<?php echo urlencode($thumb["url"]) ?>&k=<?php echo htmlspecialchars(getval("k","")) ?>&col=<?php echo $usercollection ?>" onClick="return CentralSpaceLoad(this,true);"><?php echo tidy_trim(i18n_get_translated($thumb["title"]),15) ?></a>&nbsp;</div> 
